...
func AddSession(sess *model.Session) error {
	...
	_, err := utils.Db.Exec(sqlStr, sess.SessionID, sess.UserName, sess.UserID, sess.UserIdentity)
	if err != nil {
		fmt.Println(err)
		return err
	}
...
func GetSession(sessID string) (*model.Session, error) {
	...
	sess := &model.Session{}
	row.Scan(&sess.SessionID, &sess.UserName, &sess.UserID, &sess.UserIdentity)
	return sess, nil
}
